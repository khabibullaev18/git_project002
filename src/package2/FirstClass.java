package package2;

public class FirstClass {

	public static void main(String[] args) {
		System.out.println("This is the first changes from local master");
		System.out.println("Thi is the second commit from local master");
		System.out.println("This is the third commit from local master");
		System.out.println("This is the first commit from remote repo");

		System.out.println("This is the forth commit from local master");

		System.out.println("This is the second commit from remote master");
		System.out.println("first commit from Ali branch");
		
		
		System.out.println("The second commit from local Ali");
		System.out.println("This is the third commit from local Ali");


	}

}
